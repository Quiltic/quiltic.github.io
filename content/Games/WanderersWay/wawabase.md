...
Welcome to Wanderer's Way. A game based on Powered by the Apocalypse with concepts and combat taken from a previous game of mine RPG-Hell (Or Foglight depending on version).
While this game is set to the universe concept of The Wanderer, described in more details below, you can re-flavor anything to fit your specific universe.
...
> Max/Min stat is +3/-3
>
> Check = 2d6+Stat, x<=6 -> Fail. x=7~9 -> Partial, x>=10 -> Pass
>
...
This value is calculated by rolling 1d6. Players always win if speeds are the same.
...
At the beginning of your turn you roll 5d6.
...
: For example, the Technician's Oil Patch requires you to give it ⚂⚂⚂, or 3 dice. These dice values do not matter as you will always do its effect. On the other hand the Treasure Hunter's Eye on the Prize wants ⚂, 1 dice, but on a 6 it becomes better.
...
**Shielding** - You may spend a Shielding to ignore all damage from an Attack or Effect.
**Dodge** - When you are attacked roll 2d6+Dodge. If you beat the given value you take no damage.
...
Step 1, Pick a base
 ...
 - 1 hand, 1 dmg, Near
 - 2 hand, 1 dmg, Far
Step 2, Pick a Special
 - Range is Reaching (must be Close)
 - Near Throw Range (must be Close)
 - does Elemental dmg
 - no downside
 - Does double dmg to obj and structures
Step 3, Pick a bonus (x<=7)
 - +1 dmg
 - +4 Knockback
 - Range is Reaching (must be Close)
 - refund 1 Strain
 ...
 - Remove 1 Shield from target then do dmg
Step 4, Pick a Downside (x>=11)
 - -1 dmg
...
Burn - Start of turn roll 2d6. If you roll less than the amount of Burn take 1 Damage. End of turn lose 1 Burn.
Empowered - Get +1 Ongoing and to Attack Bonus threshold (8 instead of 7).
...
Stats - Physique, Finesse, Ingenuity, Spirit, Presence, Awareness
Set stats based on +1,+1,0,0,-1,-1 (can stack the +'s and -'s for +2,-2, or another 0)

Pick a [Playbook](playbooks.md). Then 2 Traits, either from the chosen Playbook or another one. You do not get the default Trait if you use a Trait from a different playbook (though you may still pick a default Trait as a chosen Trait).

Pick 3 [Arts](arts.md). We recommend one from each category though you are not required to do so.
